...
listint_t *add_nodeint_end(listint_t **head, const int n)
{
	...
	if (new_node == NULL)
		return (NULL);
	/* access the n field iof the struct and initialize it with n */
	/* which has been passed to the function */
	new_node->n = n;
	/* since we are adding the end of the list then the */
	/* since filed should point to the end of the current list ie NULL */
	new_node->next = NULL;
