...
listint_t *insert_nodeint_at_index(listint_t **head, unsigned int idx, int n)
{
	...
	if (new_node == NULL)
		return (NULL);
	/* access the n filed of the new_node and initialize it as n */
	new_node->n = n;
	/* check if idx = 0 */
	if (idx == 0)
	{
		/* access the next field of new_node assign it as first node */
		new_node->next = *head;
		*head = new_node;
		return (new_node);
	}
	/* make reaverse be the value at head */
	traverse = *head;
